...
if (submitcheck('listvaluesubmit')) {
	...
	if(!empty($_POST['operation'])) {
		...
		$newidarr = array();
		$tablename = $_SGET['folder'] ? 'postitems' : 'spaceitems'; // 发布后使用spaceitems, 待审时使用postitems
		...
		switch($_POST['operation']) {	//跟据操作类型做相应的操作处理
			...
			case 'movefolder':	//移动文件夹
				if(!$_SGET['folder']) {	//发布箱->待审箱
					if($_POST['opfolder'] == 2) {
						deleteitemhtml($itemidarr);		//删除已生成的HTML文件
						moveitemfolder($itemidstr);  //缺省是从0到1,从发布箱到待审箱
					}
				} else {	//待审箱->发布箱
					if($_POST['opfolder'] == 1) {
						moveitemfolder($itemidstr, 1, 0); //从1到0是从待审箱到发布箱
					}
				}
				break;
	...
	if(empty($itemid) || $page == 1) {
		...
		if(empty($itemid)) {
			...
			postspacetag('update', $type, $itemid, $tagarr, $status=1); //bug fix for tag error in news editer
		}
		...
		$setsqlarr['relativeitemids'] = getrelativeitemids($itemid); // bug fix for tag error in news editor
...
if(empty($_GET['op'])) {
	...
	if(!$_SGET['folder']) {   //folder为0时（发布箱）
		...
		if(empty($thevalue['hash'])) $thevalue['hash'] = $hashstr; //(CSIP) fix a bug for not uploading attachment in 待审箱 
	}
	$type = $thevalue['type'];
