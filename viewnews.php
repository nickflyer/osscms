...
if(!empty($news['newsurl'])) {
	sheader(htmlspecialchars_decode($news['newsurl']));  // CSIP change, 加入对url中特殊符的处理
}
...
//CSIP,调用上一篇资讯标题
$sql_p = 'SELECT subject FROM '.tname('spaceitems').' WHERE itemid < \''.$itemid.'\' AND catid = \''.$thecat['catid'].'\' AND folder=\'1\' AND type=\'news\' ORDER BY itemid DESC LIMIT 0,1';
//echo $sql;
$title_p = $_SGLOBAL['db']->result($_SGLOBAL['db']->query($sql_p), 0);

if(!$title_p) $title_p = 'None';

//CSIP,调用下一篇资讯标题

$sql_n = 'SELECT subject FROM '.tname('spaceitems').' WHERE itemid > \''.$itemid.'\' AND catid = \''.$thecat['catid'].'\' AND folder=\'1\' AND type=\'news\' ORDER BY itemid DESC LIMIT 0,1';
$title_n = $_SGLOBAL['db']->result($_SGLOBAL['db']->query($sql_n), 0);

if(!$title_n) $title_n = "None";
