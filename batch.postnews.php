...
$setid = empty($_POST['setid']) ? (empty($_GET['setid']) ? 1 : intval($_GET['setid'])) : intval($_POST['setid']);
...
{	
	if('$message_id') {
		var msg = document.getElementById('$message_id').innerHTML;
	}
	
	if('$subject_id') {
		var subject = document.getElementById('$subject_id').innerHTML;
	}
	...
	sbtn += '$strbtn';
	sbtn += '</form>';
	document.getElementById('ss_btn').innerHTML = sbtn;
}
