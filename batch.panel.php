...
//				$showposturl = $siteurl.'/cp.php?ac=news&op=add&type='.$value['nameid'];
                                $showposturl = $siteurl.'/admincp.php?action=spacenews&type=news&op=add'; //The position of url to add news!!!
	...
	function contributeop() {
		if(get_id('contribute_op').style.display != 'block') {
			get_id('contribute_op').style.display = 'block';
		} else {
			get_id('contribute_op').style.display = 'none';
		}	
	}
	function hidendivop(){
		get_id('contribute_op').style.display = 'none';
	...
	function hidenposition(){
		get_id('loginin_info').style.display = get_id('loginin_info').style.display=='none' ? '' : 'none';
		get_id('loginin_info_all').style.display = get_id('loginin_info_all').style.display=='none' ? '' : 'none';
	}
	...
	document.write('<div class="user"><a href="javascript:;" onclick="hidenposition();" title="$blang[open]">$_SGLOBAL[supe_username]</a> <a class="credit" href="$siteurl/cp.php?ac=credit" title="$lang[credit]">{$_SGLOBAL['member']['credit']}</a>&nbsp;<span class="out">[<a href="$siteurl/batch.login.php?action=logout">$blang[safe_logout]</a>]</span></div>');
	document.write('<span class="admin">');
	document.write(' <a href="$siteurl/space.php?uid=$uid">$blang[my_center]</a>');
	document.write('</span>');
	...
	document.write('<a class="tx_blue" href="$_SC[uchurl]/space.php?uid=$uid">$blang[my_space]</a>');
	document.write(' <a class="tx_blue" href="$siteurl/cp.php">$blang[my_conf]</a>');
	document.write('</dd>');
	document.write('</dl>');
