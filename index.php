...
if(!empty($channels['default']) && $channels['default'] != 'index.php') {
	...
	unset($value);
	//$_SGET['time'] = '672';
	$_SGET['time'] = '1512'; //CSIP,获取两个月内的排行，小时数
	$time = $_SGET['time'] ? $_SGLOBAL['timestamp'] - $_SGET['time'] * 3600 : 0;
	$setwhere = $time ? " WHERE dateline >= '$time'" : '';
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('spaceitems')." $setwhere ORDER BY viewnum DESC, dateline DESC LIMIT 8");
	$i = 1;
...
$conn=mysql_connect("119.254.229.22","remoteuser","12345678"); //脕卢陆脫脢媒戮脻驴芒
if(!$conn)
{
   echo mysql_error();
}
mysql_query("set names 'utf8'"); //艙芒鸥枚脰脨脦脛脗脪脗毛

mysql_select_db("yp",$conn); //脩隆脭帽脢媒鸥脻驴芒

$sql="select resource_title from sw_resource"; //sql脫茂鸥盲
